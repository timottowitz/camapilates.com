...
const CityDirectory: React.FC = () => {
  ...
  const convexStudios = useQuery(
    api.studios.getByCity,
    hasConvex && cityName ? { city: cityName } : "skip",
  );
  const convexCity = useQuery(
    api.cities.getBySlug,
    hasConvex && city ? { slug: city } : "skip",
  );
  ...
  const fallbackStudios = (localData.studios as any[]).filter(
    (s: any) =>
      (s.address?.city || "").toLowerCase() === (cityName || "").toLowerCase(),
  );
  const fallbackCityData = (localData.cities as any[]).find(
    (c: any) => c.slug === (city || ""),
  );

  const remoteStudios = Array.isArray(convexStudios)
    ? convexStudios
    : undefined;
  ...
  const filteredStudios = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter((studio) => {
        ...
        const matchesClassType = studio.classTypes?.some((type) =>
          (type ?? "").toLowerCase().includes(term),
        );

        return (
          normalizedName.includes(term) ||
          normalizedNeighborhood.includes(term) ||
          Boolean(matchesClassType)
        );
      });
    }
    ...
    switch (sortBy) {
      case "rating":
        result.sort(
          (a, b) =>
            (b.metrics?.googleRating || 0) - (a.metrics?.googleRating || 0),
        );
        break;
      case "reviews":
        result.sort(
          (a, b) =>
            (b.metrics?.googleReviewCount || 0) -
            (a.metrics?.googleReviewCount || 0),
        );
        break;
      case "price-low":
        result.sort(
          (a, b) =>
            (a.pricing?.singleClassMin || 9999) -
            (b.pricing?.singleClassMin || 9999),
        );
        break;
      case "price-high":
        result.sort(
          (a, b) =>
            (b.pricing?.singleClassMin || 0) - (a.pricing?.singleClassMin || 0),
        );
        break;
      case "quality":
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (
      ...
      filters.priceRange[1] < availableOptions.maxPrice
    )
      count++;
    return count;
    ...
    <LuxuryLayout>
      ...
      <section className="relative pt-32 pb-12 px-8 md:px-24 max-w-[1800px] mx-auto">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-[#5D5550] hover:text-[#2A2624] mb-12 transition-all hover:gap-3 group"
        >
            ...
            <div className="flex items-center justify-center gap-8 text-sm text-[#5D5550]">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">
                  {displayStudioCount} estudios
                </span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-[#D9865B] rounded-full" />
                <span className="font-medium">
                  {cityData.neighborhoods?.length ?? 0} colonias
                </span>
              </div>
            </div>
          )}
        ...
        <div className="lg:grid lg:grid-cols-4 lg:gap-12">
          ...
          <div className="lg:col-span-3">
            ...
            <div className="flex items-center justify-between mb-8 pb-6 border-b-2 border-[#2A2624]/10 relative">
              ...
              <div className="flex items-center gap-4">
                <h2 className="text-2xl font-serif italic text-[#2A2624] tracking-tight">
                  <span className="font-bold">{filteredStudios.length}</span>{" "}
                  {filteredStudios.length === 1 ? "Estudio" : "Estudios"}
                </h2>
              ...
              <div className="flex items-center gap-3">
                <span className="text-xs text-[#5D5550] hidden sm:inline uppercase tracking-widest">
                  Ordenar:
                </span>
                <Select value={sortBy} onValueChange={setSortBy}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="price-low">
                      ð° Precio: menor a mayor
                    </SelectItem>
                    <SelectItem value="price-high">
                      ð Precio: mayor a menor
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {filters.neighborhoods.map((n) => (
                  <Badge
                    key={n}
                    variant="secondary"
                    className="bg-[#2A2624]/5 text-[#2A2624] hover:bg-[#2A2624]/10"
                  >
                    {n}
                    <button
                      onClick={() =>
            ...
            <Tabs
              value={viewTab}
              onValueChange={(value: any) => setViewTab(value)}
              className="w-full"
            >
                ...
                <div className="mt-0">
                  <StudioMap
                    ...
                    onStudioClick={(studio) => {
                      navigate(
                        `/estudios-de-pilates/${citySlugNormalized}/${studio.slug}`,
                      );
                    }}
                  />
                </div>
