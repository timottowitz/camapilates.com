...
const TrustStrip: React.FC<{ className?: string }> = ({ className = '' }) => {
  return (
    <div className={`flex flex-wrap items-center gap-3 text-xs ${className}`}>
      ...
      <div className="flex items-center gap-1 text-foreground"><Truck className="h-3.5 w-3.5 text-primary" /> Entrega 3 semanas (MX)</div>
      <div className="flex items-center gap-1 text-foreground"><ShieldCheck className="h-3.5 w-3.5 text-primary" /> Garantía 1 año</div>
