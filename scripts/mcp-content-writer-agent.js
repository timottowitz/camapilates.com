  ...
  tools: [
    {
      ...
      inputSchema: {
        ...
        properties: {
          slug: {
            type: 'string',
            description: 'Blog slug (must have corresponding research file in blog-planning/research/)'
          },
          forceOverwrite: {
            type: 'boolean',
            default: false,
...
- Público objetivo: ${metadata.targetAudience || 'Mercado mexicano de Pilates'}
...
- Recomendaciones CAMA Pilates (100-150 palabras)
- FAQ (5-8 preguntas)
...
FORMATO DE RESPUESTA (JSON):
{
  ...
  "mexican_context": ["CDMX", "Guadalajara", "Monterrey", "MXN pricing"],
...
- Menciona contexto mexicano (CDMX/GDL/MTY, precios MXN)
...
- Usa ejemplos mexicanos específicos
...
async function generateFAQs(researchContent, metadata, outline) {
  ...
  const systemPrompt = `Eres un experto en Pilates creando contenido FAQ para consumidores mexicanos.

ESTILO FAQ:
- Pregunta: directa y conversacional ("¿Cuál...?", "¿Cómo...?")
- Respuesta: 80-120 palabras, práctica y específica
- Incluye datos mexicanos (precios MXN, ciudades, disponibilidad)
...
CONTEXTO MEXICANO:
${outline.mexican_context?.join(', ') || 'México, CDMX, Guadalajara, Monterrey'}
...
- 1-2 preguntas de mercado mexicano (precios, disponibilidad)
...
4. Gramática y ortografía en español mexicano
...
function assembleContent(sections, faqs, metadata) {
  ...
  // CAMA Pilates recommendations section
  const camaSection = sections.find(s => s.heading.toLowerCase().includes('cama pilates'));
  if (!camaSection) {
