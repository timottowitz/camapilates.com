...
async function main() {
  ...
  const pending = await client.query(api.placeholders.list, { status: 'pending' }).catch(() => []);
  const prompts = await client.query(api.placeholders.list, { status: 'prompt_generated' }).catch(() => []);
  const todos = [...pending, ...prompts];
  console.log(`Found ${todos.length} placeholders to generate`);
  let queued = 0, failed = 0;
  for (const p of todos) {
    try {
      await client.action(api.placeholderGeneration.queue, { placeholderId: p.placeholderId });
      queued++;
    } catch (e) {
      failed++;
