  ...
  handler: async (ctx, { queries, limit }) => {
      ...
      for (const q of seeds) {
        try {
          ...
          (j?.data?.children || []).forEach((child: any) => {
            ...
            pool.push({ title, url: `https://reddit.com${d?.permalink || ''}`.replace(/\/$/, ''), score });
          });
        } catch {}
      }
    }
    ...
    async function ddg(query: string) {
      try {
        ...
        const resp = await fetch(url, { headers: { 'user-agent': 'Mozilla/5.0 CAMA-Pilates-Convex/1.0' } });
        ...
        return results;
      } catch { return [] as Array<{ title: string; url: string }>; }
    }
